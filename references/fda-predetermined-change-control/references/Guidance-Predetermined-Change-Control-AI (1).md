# Guidance-Predetermined-Change-Control-AI (1)
...
```unknown
important part of many medical devices. This guidance is intended to
```

```unknown
include software functions excluded pursuant to section 520(o)” of
```
...
```unknown
used in accordance with the definitions of manufacturer in 21
```
...
```elixir
use populations, which is referred to as the
```

```yaml
includes: 1) a detailed description of the specific, planned device modifications, which
```
...
```elixir
use of AI in radiological imaging, and
```

```elixir
use of a device as such device changes pursuant to such plan, notification
```

```unknown
users of AI/ML-enabled medical devices and ways in which transparency might enhance the
```

```unknown
include in the labeling and
```

```unknown
important pillar of the Action Plan is the further advancement of the tailored regulatory
```

```unknown
require that a PCCP include labeling
```
...
```elixir
use populations. Specifically, this guidance includes
```
...
```unknown
require human input, action, review, and/or decision-making,
```

```unknown
included in the PCCP in a marketing
```

```unknown
includes those device modifications that generally would
```

```unknown
require a new marketing submission.14,15 These modifications include those that could
```

```unknown
includes a PMA application, 510(k) submission,
```

```unknown
require a new marketing submission pursuant to 21 CFR 807.81(a)(3) and 21
```

```unknown
require a new marketing submission, see the Device
```
...
```unknown
include 1) those that “could significantly affect the safety or effectiveness of the device, e.g., a significant
requirements, the broader 
...
```elixir
use a PCCP is appropriate for device modifications that generally would otherwise require a
```
...
```typescript
required as a condition to approval of the device.25
```

```unknown
require a new marketing submission. For such modifications, the
```

```unknown
requires manufacturers of finished
```

```unknown
requirements, such modifications need to be reported to FDA in post-
```

```unknown
required for “change[s] affecting the safety or effectiveness of the device” unless an exception
```

```unknown
includes approval of a PMA application, clearance of a
```

```unknown
requirements in Part 820 and instead incorporate
```
...
```sql
requirements, in making a determination of substantial equivalence where the
```

```unknown
include in a marketing submission for an AI-DSF.32 The
```
...
```unknown
requirements and the requirements for valid scientific evidence.33
```
...
```elixir
use model inference to formulate options for information or action.34
```

```elixir
use or a subset of the intended use
```

```typescript
include Software as a Medical Device (SaMD) and Software in a Medical Device
```

```unknown
used for purposes of this guidance.
```

```unknown
used by the manufacturer of an AI-DSF in procedures and
```
...
```elixir
use populations (e.g., with respect to race, ethnicity, disease severity, sex, age,
```
...
```unknown
used by the manufacturer of an AI-DSF to evaluate a
```

```unknown
used to characterize the performance of an AI-DSF. These data are
```

```unknown
used to estimate the AI model’s
```
...
```unknown
include device modifications that would otherwise require a
```
...
```unknown
includes the verification and
```

```unknown
included in the PCCP
```
...
```elixir
use of the device or could
```

```elixir
use modifications that are specified and implemented in accordance with an authorized
```

```unknown
included in a PCCP, could
```

```unknown
require a new marketing submission pursuant to 21 CFR 807.81(a)(3) and 21 CFR
```
...
```unknown
require a new marketing submission prior to
```

```elixir
use populations; and
use the device to be adulterated and misbranded under
```
...
```elixir
use these components are
```

```elixir
use populations. The Impact Assessment helps to tie the
```

```sql
includes marketing authorization for a device and PCCP where the device or a derivative thereof has yet to
```

```unknown
use, so that the device continues to reflect these populations and
```

```unknown
includes defining the
include a PCCP, because a PCCP 
...
```unknown
included in an authorized PCCP, FDA expects
```

```unknown
used in, or the facilities or controls used for, the manufacture, processing,
```

```unknown
requirements. Thus, consistent
```

```unknown
used in, or the facilities or controls
requirements, the determination of substantial equivalence 
...
```elixir
use of the subject device and the predicate device are the same, to help
```

```typescript
included as a standalone section within the marketing submission, with a
```
...
```unknown
includes modifications that have been implemented consistent with the authorized PCCP has
```
...
```unknown
included in the marketing submission’s table of contents as “Predetermined
```
...
```elixir
include labeling required for safe and effective use of the device
```

```elixir
important in order for a user to use the device
```

```elixir
use the device safely and effectively across the intended use populations and
```

```elixir
use or a version history.
```

```elixir
use the device safely and for the purpose for which
```

```unknown
used for the assessment of reasonable
```

```unknown
included outside of the PCCP section should include appropriate
```

```unknown
require the user to perform software updates, and that such
```

```unknown
user to understand changes in the device and
```

```unknown
include a statement that the device has an authorized PCCP.
```
...
```unknown
users may be aware of
```

```unknown
users will be informed of implemented modifications,
```

```unknown
include adequate directions for use); 21 CFR 801.109(c) (for prescription
```

```unknown
include any relevant hazards, contraindications, side effects, and precautions under
```

```unknown
use. Information on the AI-
```
...
```typescript
include a summary of the following information regarding the PCCP, as appropriate:
```
...
```unknown
users will be informed of device modifications implemented in
```

```unknown
requirements regarding the device and its authorized PCCP. In general, a
```
...
```unknown
users of FDA’s determination of
```

```unknown
requirements on the content and format of a 510(k) Statement.
```

```unknown
required to comply with the QSR (21 CFR Part 820). The device and PCCP must be
```
...
```unknown
requirements to document changes and approvals in the device master record (21 CFR 820.181).
```
...
```elixir
use modifications included in a PCCP are those that
```

```unknown
included in the authorized PCCP or if the modification is included in the
```

```unknown
included in the authorized PCCP, the manufacturer should
```

```unknown
requirements and after consulting the Device
```

```unknown
included in the authorized PCCP but is not implemented in
```

```unknown
require a new marketing submission, it is likely that a new
```

```unknown
required before the manufacturer can implement the
```

```unknown
require a new marketing
```

```unknown
requirements a new marketing
```

```unknown
required,71 the appropriate marketing submission could request authorization for:
```

```unknown
include a change in the Description of Modifications, the Modification
requirements73 for the device. If the manufacturer requests
```
...
```elixir
use a modification to the PCCP will generally
```

```unknown
require a new marketing submission.77 In other words, FDA
include the appropriate marketing submission 
...
```unknown
included in Section V.B. of this guidance to be
```

```unknown
requirements, a special 510(k) submission may be appropriate to modify a PCCP where
```

```unknown
includes a PCCP will be interactive. It is
```

```unknown
included in the marketing submission for the device. If the PCCP is revised
```

```unknown
include a title and current version number for the
included in their authorized PCCP and that may 
...
```unknown
include the appropriate marketing submission requirements88 and
```

```unknown
requirements, a manufacturer must compare their subject device to
```

```unknown
includes modifications that have been implemented
```
...
```unknown
include the specifications for the characteristics and performance of the
```
...
```elixir
use this guidance includes
```

```typescript
included in the Modification Protocol, as described in Section VII.B.(4)
```

```unknown
include modifications that are specific, and that can be verified
```

```unknown
include information regarding the expected frequency
```

```unknown
use. Finally, it may be
```
...
```elixir
use and performance (e.g., for use within
```

```elixir
use population from the same type and range of input
```

```elixir
use and performance could include authorization of a
```

```elixir
use included in a PCCP would be
```

```typescript
included in the Modification Protocol, as described in Section VII.B.(4) of
```

```typescript
use.93 As with modifications to the intended use,
```
...
```unknown
include changes to data type
```

```unknown
include new sources of the same input signal type (e.g., different makes,
```

```unknown
include updates related to available, compatible software or hardware and
```

```unknown
included in an authorized PCCP for various AI-DSFs.
```

```unknown
included in a PCCP must maintain the device within the device’s intended use,91
```

```unknown
included in a PCCP should also maintain
```

```elixir
use in the PMA regulation at 21 CFR
use to specify use of the device with an additional device or
```
...
```elixir
use that may be included in a
```

```unknown
include the documentation describing the methods that will be
```
...
```unknown
use (e.g., certain
used to develop, validate, and implement all proposed
```
...
```unknown
users for implemented modifications;
```

```unknown
requirements and in accordance with the manufacturer’s
```
...
```unknown
included to assist in FDA’s determination of substantial
```

```unknown
include a description of how its
```

```unknown
used elsewhere in the marketing
```

```unknown
include references to other marketing submissions
```

```unknown
used for similar modifications.
```
...
```elixir
use of synthetic or augmented data. To support
```

```elixir
use training, tuning, and test data that are complete and
```

```elixir
use populations (e.g., with respect to race, ethnicity,
```

```sql
used to test the AI-DSF is separate and independent from the development process used to
```

```unknown
used by the device and often utilize a label or ‘reference
```

```unknown
used for training and tuning) to prevent overfitting and
```

```unknown
used to develop the initial AI-DSF) will be collected. The data
```
...
```unknown
used to train, tune, and test the initial and any subsequent
```

```unknown
used to support each modification to an AI-DSF, including 1)
```

```unknown
requires manufacturers to retain all records for a period of time equivalent to the design and expected
```
...
```typescript
used to generate data as inputs for the AI-DSF,
```

```sql
used, and clinical scenarios where the device will
```

```unknown
include in a submission: Examples of the types of information
```
...
```unknown
included in this component of the Modification
```
...
```elixir
use will be applied for testing; what performance metrics will be computed; and
```

```elixir
use analysis of the failure reveals it is not related to
```

```typescript
include, as applicable, the plans for verification and validation of the entire device
```

```unknown
require that performance requirements for changes made under the
```

```unknown
includes, but is not limited to, AI model testing protocols comparing the
```

```unknown
used to evaluate the effect of modifications on overall device
```

```unknown
important to ensure that specified
```

```unknown
include in a submission: Examples of the types of information
include real-world monitoring and notification requirements if the device does not function
```
...
```elixir
use the device. The update procedures should clearly describe which planned
```

```elixir
use it could cause confusion and would be misleading.
```

```elixir
include adequate directions for use including statements of all
```

```unknown
users about the modifications that is needed
```

```unknown
users and updated user training.104 The
```

```unknown
include adequate directions for use.107 The
```

```unknown
user, including information regarding site-specific modifications. New unique
```

```unknown
required for devices that are required to bear a UDI on its label
```

```unknown
include information on modifications to the AI-DSF that have not been
```

```unknown
users); 3) how the device
```
...
```unknown
uses for which the device is intended.
```

```unknown
users. The update
users will be kept up-to-
```
...
```unknown
users, including transparency on any differences in performance or device
```

```unknown
important for FDA to
```
...
```typescript
used as the framework in which to conduct an
```

```unknown
important to address these elements in an Impact Assessment in order to
```

```unknown
included in the PCCP impact not only the AI-DSF, but also how
```

```unknown
included in the PCCP for the device
```

```unknown
important to determine if any information should be included in
```

```unknown
included elsewhere in the marketing
```
...
```typescript
included as part of a PCCP in a marketing submission
```

```unknown
include four components that outline a manufacturer’s 1) data management practices, 2)
```
...
```unknown
include in a marketing submission, this appendix includes
```

```unknown
included in a PCCP for some device types so that FDA can make a
```

```unknown
used for training, tuning, and testing AI model updates. In
```

```unknown
include how the data will be used (e.g., for AI model development or testing),
```

```unknown
required in the marketing submission pursuant to e.g., sections 513(i)(1) and 515C of
```
...
```elixir
use protocols in compliance with
```

```elixir
use input data from dedicated acquisition systems (e.g.,
```
...
```unknown
uses connected sensors), are
```

```unknown
used to promote data authenticity, transparency, and
...
```unknown
include consideration of device risk.
```

```unknown
used during training, tuning, testing, or all three.
```

```unknown
used (e.g., what is the total number of clinicians who
```

```unknown
used to support the initial
```

```unknown
used multiple times for performance evaluation, what
```
...
```unknown
used to support labeling specifications?
```

```unknown
used)? When the reference
```

```unknown
includes a diagnostic error), are errors
```

```unknown
used to develop the acceptance criteria?
```

```unknown
user need to interact with the device to evaluate the
```

```unknown
used for the version of the device without any
```

```unknown
used to control a device, or the
```
...
```sql
users have the option to review labeling before implementing an update or be
```

```unknown
includes other device functions in addition to the AI-DSF, how will
```

```unknown
includes “other functions” in addition to the AI-DSF, how will the
```

```unknown
used, manually by users or hospital technicians, or both
```

```unknown
users, including, but not limited to, in
```

```unknown
user when reviewing device
```
...
```elixir
use is available and can be rerun on an
```

```unknown
user, including, but not limited to, in labeling?
```

```unknown
users? Is there a plan to
```

```unknown
users on the performance of
```

```unknown
users, will information about monitoring real-world device
```

```unknown
users about the safety performance of
```
...
```elixir
use in high-acuity healthcare environments (e.g., an
```

```unknown
included in the PCCP and has been implemented in accordance with the
```
...
```unknown
important concepts that could inform the development and utility of a PCCP, the PCCP
used to re-train the AI
```
...
```unknown
users. Because the device
```

```unknown
require a new marketing submission. The manufacturer
```

```unknown
included in the PCCP. The methods used for analysis, performance,
```

```unknown
included in the PCCP could significantly affect the safety or
```
...
```elixir
use on additional general-purpose
```

```elixir
use the device modification was specified in the PCCP, and it was implemented in
```

```elixir
use this modification that was not included in the PCCP could significantly affect the safety
```

```elixir
use and performance, which was
```

```unknown
used by a primary healthcare provider. The
```

```unknown
include a two-dimensional camera that meets the minimum specifications defined
```
...
```elixir
use this modification that was not included in the PCCP could
```

```elixir
use in the healthcare or home-use setting. The AI-DSF
```

```elixir
use and performance in a subset of
```

```elixir
use the device modification was specified in
```

```unknown
used to validate the updated AI model. The AI-DSF was updated to
```
...
```elixir
use and performance in a subset of the
```

```elixir
use the manufacturer had to implement a reference standard
```

```elixir
use this modification that was not implemented in conformance with the PCCP could
```

```unknown
require a new marketing submission. The manufacturer should document the
```

```unknown
users. Because the device modification was specified in the PCCP, and because it was
```
...
```elixir
use and performance, as specified in
```

```elixir
use the device modification was specified in the PCCP, and it
```

```sql
used new images to re-train the AI model and would like to update their
```

```unknown
use. However, the modification was
```
...
```elixir
use and performance, which was not
```
...
```unknown
used the same database of images to re-train the AI model to identify
```
...
```elixir
included in the intended use population. The modification was not
```
...
```sql
uses data from a patient
```

```unknown
use, which was not specified in the
```
